...
SARDET50K_SPLIT = dict(
    ALL_CLASSES=('ship', 'aircraft', 'car', 'tank', 'bridge', 'harbor'),
    NOVEL_CLASSES=('aircraft',),
    BASE_CLASSES=('ship', 'car', 'tank', 'bridge', 'harbor'))


@DATASETS.register_module()
class FewShotSARDet50KDataset(BaseFewShotDataset, CocoDataset):
    ...
    Args:
        classes (str | Sequence[str] | None): Classes for model training and
            ...
            it will load pre-defined classes in :obj:`FewShotSARDet50KDataset`.
    ...
    def __init__(self,
        ...
        self.SPLIT = SARDET50K_SPLIT
        assert classes is not None, f'{self.dataset_name}: classes in ' \
                                    f'`FewShotSARDet50KDataset` can not be None.'
    ...
    def get_classes(self, classes: Union[str, Sequence[str]]) -> List[str]:
        ...
        Args:
            classes (str | Sequence[str]): Classes for model training and
                provide fixed label for each class. When classes is string,
                it will load pre-defined classes in `FewShotSARDet50KDataset`.
    ...
    def evaluate(self,
        ...
        Args:
            ...
            class_splits: (list[str] | None): Calculate metric of classes split
                in SARDET50K_SPLIT. For example: ['BASE_CLASSES', 'NOVEL_CLASSES'].
                Default: None.
        ...
        for metric in metrics:
            ...
            if class_splits is not None:
                class_splits = {k: SARDET50K_SPLIT[k] for k in class_splits}
...
@DATASETS.register_module()
class FewShotSARDet50KCopyDataset(FewShotSARDet50KDataset):
    ...
    Args:
        ann_cfg (list[dict] | dict): contain `data_infos` from other
            dataset. Example: [dict(data_infos=FewShotSARDet50KDataset.data_infos)]
    """
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]) -> List[Dict]:
        ...
        Args:
            ann_cfg (list[dict] | dict): contain `data_infos` from other
                dataset. Example:
                [dict(data_infos=FewShotSARDet50KDataset.data_infos)]
...
@DATASETS.register_module()
class FewShotSARDet50KDefaultDataset(FewShotSARDet50KDataset):
    ...
    different from :obj:`FewShotSARDet50KDataset`. The `ann_cfg' should contain
    ...
    #         dict(
    #             type='ann_file',
    #             ann_file=f'data/few_shot_ann/sardet50k/sar_seed1/'
    #             f'full_box_{shot}shot_{class_name}_trainval.json')
    #         for class_name in SARDET50K_SPLIT['ALL_CLASSES']
    #     ]
    ...
    sardet50k_benchmark = {
        f'{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'data/few_shot_ann/sardet50k/'
                f'SARDet50K_{shot}shot_train.json')
        ]
        for shot in [1, 2, 3, 5, 10, 30, 1000]
    }

    # pre-defined annotation config for model reproducibility
    DEFAULT_ANN_CONFIG = dict(
        TFA=sardet50k_benchmark,
        FSCE=sardet50k_benchmark,
        Attention_RPN={
            **sardet50k_benchmark, 'Official_10SHOT': [
                dict(
                    type='ann_file',
        ...
        MPSR=sardet50k_benchmark,
        MetaRCNN=sardet50k_benchmark,
        FSDetView=sardet50k_benchmark)
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(FewShotSARDet50KDataset, self).ann_cfg_parser(new_ann_cfg)
