...
SARDET100K_SPLIT = dict(
    ALL_CLASSES_SPLIT1=('ship', 'aircraft', 'car', 'tank', 'bridge', 'harbor'),
    ALL_CLASSES_SPLIT2=('ship', 'aircraft', 'car', 'tank', 'bridge', 'harbor'),
    NOVEL_CLASSES_SPLIT1=('aircraft',),
    NOVEL_CLASSES_SPLIT2=('ship',),
    BASE_CLASSES_SPLIT1=('ship', 'car', 'tank', 'bridge', 'harbor'),
    BASE_CLASSES_SPLIT2=('aircraft', 'car', 'tank', 'bridge', 'harbor'))


@DATASETS.register_module()
class FewShotSARDet100KDataset(BaseFewShotDataset, CocoDataset):
    ...
    Args:
        classes (str | Sequence[str] | None): Classes for model training and
            ...
            it will load pre-defined classes in :obj:`FewShotSARDet100KDataset`.
    ...
    def get_classes(self, classes: Union[str, Sequence[str]]) -> List[str]:
        ...
        Args:
            classes (str | Sequence[str]): Classes for model training and
                provide fixed label for each class. When classes is string,
                it will load pre-defined classes in `FewShotSARDet100KDataset`.
    ...
    def evaluate(self,
        ...
        Args:
            ...
            class_splits: (list[str] | None): Calculate metric of classes split
                in SARDET100K_SPLIT. For example: ['BASE_CLASSES', 'NOVEL_CLASSES'].
                Default: None.
...
@DATASETS.register_module()
class FewShotSARDet100KCopyDataset(FewShotSARDet100KDataset):
    ...
    Args:
        ann_cfg (list[dict] | dict): contain `data_infos` from other
            dataset. Example: [dict(data_infos=FewShotSARDet100KDataset.data_infos)]
    """
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]) -> List[Dict]:
        ...
        Args:
            ann_cfg (list[dict] | dict): contain `data_infos` from other
                dataset. Example:
                [dict(data_infos=FewShotSARDet100KDataset.data_infos)]
...
@DATASETS.register_module()
class FewShotSARDet100KDefaultDataset(FewShotSARDet100KDataset):
    ...
    different from :obj:`FewShotSARDet100KDataset`. The `ann_cfg' should contain
    ...
    DEFAULT_ANN_CONFIG = dict(
        TFA=sardet100k_benchmark,
        FSCE=sardet100k_benchmark,
        Attention_RPN={
            **sardet100k_benchmark, 'Official_10SHOT': [
                dict(
                    type='ann_file',
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(FewShotSARDet100KDataset, self).ann_cfg_parser(new_ann_cfg)
