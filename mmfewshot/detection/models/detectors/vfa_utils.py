...
class PCB:
    """CLIP proposal classifier used by VFA's optional COCO refinement."""

    def __init__(self,
                 class_names,
                 model='RN101',
                 templates='a photo of a {}') -> None:
        ...
        self.prompts = clip.tokenize([
            templates.format(cls_name) for cls_name in self.class_names
        ]).to(self.device)
        with torch.no_grad():
    ...
    def load_image_by_box(self, img_path, boxes):
        ...
        for box in boxes:
            ...
            image_list.append(self.trans(sub_image).to(self.device))
